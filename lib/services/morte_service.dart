...
class MorteService {
  Future<List<Morte>> list({String? q}) async {
    ...
    final rows = await db.query('morte', orderBy: 'criado_em DESC');
  ...
  Future<void> salvar({
    ...
    if (morteExistenteId != null) {
      await db.update('morte', morte.toMap(),
          where: 'id = ?', whereArgs: [morteExistenteId]);
      return;
    }

    await db.insert('morte', morte.toMap());
  ...
  Future<void> deletarPorNascimento({
    ...
    final db = await AppDb.getDb();
    await db.delete('morte', where: 'id = ?', whereArgs: [morteId]);
